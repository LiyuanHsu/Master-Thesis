...
class RectangleOne(gym.Env):
    ...
    def __init__(self):
        ...
        self.obstacle_type = self.set_obstacle_type(self.obstacle_num)
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            ...
            r = 6.0
            # self.goal_pos[0] = r * np.cos(angle_goal)
            # self.goal_pos[1] = r * np.sin(angle_goal)

            # obstacle pose
            self.obstacle_dim = []

            self.obstacle_type = self.set_obstacle_type(self.obstacle_num)
            print ("obstacle_type:", self.obstacle_type)
    ...
    def laser_readings(self):
        ...
        lidar_readings = 10 * np.ones(num_samples)
    ...
    def set_obstacle_type(self, obstacle_num):

        obstacle_type = []
        for it_obstacle in range(obstacle_num):
            type = np.random.randint(2, size=1)[0]
            if type == 0:
                obstacle_type.append('cylinder')
            else:
                obstacle_type.append('rectangle')
