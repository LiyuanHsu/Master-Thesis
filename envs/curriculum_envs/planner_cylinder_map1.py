...
class PlannerCylinderMap1(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.max_measurement_laser = 1.0
        ...
        self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'cylinder', 'cylinder', 'cylinder']
    ...
    def _step(self, action):
        ...
        # observation = np.concatenate((observation, observation))
        #
        # goal_measurements = np.concatenate((goal_measurements, goal_measurements))
        # goal_measurements = np.concatenate((goal_measurements, goal_measurements))

        # measurement = np.concatenate((observation, goal_measurements)) # After (fedd observation first)
    ...
    def action_2_state_d(self, action):
        ...
        # return {
        #     0: np.multiply([np.cos(angle_inc * 0), np.sin(angle_inc * 0), 0.0], step_size),
        #     1: np.multiply([np.cos(angle_inc * 1), np.sin(angle_inc * 1), 0.0], step_size),
        #     2: np.multiply([np.cos(angle_inc * 2), np.sin(angle_inc * 2), 0.0], step_size),
        #     3: np.multiply([np.cos(angle_inc * 3), np.sin(angle_inc * 3), 0.0], step_size),
        #     4: np.multiply([np.cos(angle_inc * 4), np.sin(angle_inc * 4), 0.0], step_size),
        #     5: np.multiply([np.cos(angle_inc * 5), np.sin(angle_inc * 5), 0.0], step_size),
        #     6: np.multiply([np.cos(angle_inc * 6), np.sin(angle_inc * 6), 0.0], step_size),
        #     7: np.multiply([np.cos(angle_inc * 7), np.sin(angle_inc * 7), 0.0], step_size),
        #     8: np.multiply([np.cos(angle_inc * 8), np.sin(angle_inc * 8), 0.0], step_size),
        #     9: np.multiply([np.cos(angle_inc * 9), np.sin(angle_inc * 9), 0.0], step_size),
        #     10: np.multiply([np.cos(angle_inc * 10), np.sin(angle_inc * 10), 0.0], step_size),
        #     11: np.multiply([np.cos(angle_inc * 11), np.sin(angle_inc * 11), 0.0], step_size),
        #     12: np.multiply([np.cos(angle_inc * 12), np.sin(angle_inc * 12), 0.0], step_size),
        #     13: np.multiply([np.cos(angle_inc * 13), np.sin(angle_inc * 13), 0.0], step_size),
        #     14: np.multiply([np.cos(angle_inc * 14), np.sin(angle_inc * 14), 0.0], step_size),
        #     15: np.multiply([np.cos(angle_inc * 15), np.sin(angle_inc * 15), 0.0], step_size),
        # }[action]
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            ...
            self.goal_pos[0] = np.random.uniform(1, 8, 1)
            self.goal_pos[1] = np.random.uniform(1, 8, 1)

            # for i in range (50):
            #     self.goal_pos[0] = np.random.uniform(0.5, 8.5, 1)
            #     if not (self.goal_pos[0] > 2.8 and self.goal_pos[0] < 4.2):
            #         break
            #
            #
            # for i in range(50):
            #     self.goal_pos[1] = np.random.uniform(0.5, 8.5, 1)
            #     if not ((np.linalg.norm(self.goal_pos[1] - 2) < 0.3) or (np.linalg.norm(self.goal_pos[1] - 4.7) < 0.3) or (np.linalg.norm(self.goal_pos[1] - 6) < 0.3)):
            #         break
            ...
            for i in range(50):
                ...
                if np.linalg.norm(self.obstacle_pos[4, 0:2] - self.goal_pos) < 0.8 or np.linalg.norm(self.obstacle_pos[5, 0:2] - self.goal_pos) < 0.8 or np.linalg.norm(self.obstacle_pos[6, 0:2] - self.goal_pos) < 0.8:
                    continue
                else:
                    break

            print('goal_pos: ({},{})'.format(self.goal_pos[0], self.goal_pos[1]))



            # Start pose
            for i in range(50):
                # self.state[0] = np.random.uniform(0.5, 8.5, 1)
                # self.state[1] = np.random.uniform(0.5, 8.5, 1)
                ...
                if np.linalg.norm(self.state[:2] - self.goal_pos) < 2.0 or np.linalg.norm(self.obstacle_pos[4, 0:2] - self.state[:2]) < 0.6 or np.linalg.norm(self.obstacle_pos[5, 0:2] - self.state[:2]) < 0.6 or np.linalg.norm(self.obstacle_pos[6, 0:2] - self.state[:2]) < 0.6:
                    continue
                else:
                    break


                # overlap = self.test_collision(self.goal_pos, 0.2)
            ...
            for it_obstacle in range(self.obstacle_num):
                if self.obstacle_type[it_obstacle] == 'cylinder':
                    ...
                    self.obstacle_pos[4, 0:2] = [5.0, 3.0]
                    self.obstacle_pos[5, 0:2] = [3.0, 5.0]
                    self.obstacle_pos[6, 0:2] = [7.2, 6.2]

                    # self.obstacle_pos[4, 0:2] = [5.0, 5.0]
                    # self.obstacle_pos[5, 0:2] = [3.8, 2.5]
                    # self.obstacle_pos[6, 0:2] = [7.0, 5.5]
                    ...
                    # self.obstacle_pos[4, 0] = 3
                    # self.obstacle_pos[4, 1] = 1
                    #
                    # self.obstacle_pos[5, 0] = 4.5
                    # self.obstacle_pos[5, 1] = 2
                    #
                    # self.obstacle_pos[6, 0] = 1.5
                    # self.obstacle_pos[6, 1] = 4.7
                    #
                    # self.obstacle_pos[7, 0] = 4
                    # self.obstacle_pos[7, 1] = 7.5
                    #
                    # self.obstacle_pos[8, 0] = 7.5
                    # self.obstacle_pos[8, 1] = 6
                    ...
                    self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]), np.array([9.2, 0.2])]
