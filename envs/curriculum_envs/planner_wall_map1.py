    ...
    def _step(self, action):
        ...
        if done:
            ...
            if collision:
                ...
                if np.linalg.norm(self.goal_pos - self.state[0:2]) < 0.7:
                    reward = 1.0
                    done = True
                else:
        ...
        if not self.manual_pose:
            ...
            for it_obstacle in range(self.obstacle_num):
                if self.obstacle_type[it_obstacle] == 'cylinder':
                    ...
                    self.obstacle_pos[4, 1] = 3

                    self.obstacle_pos[5, 0] = 6
                    self.obstacle_pos[5, 1] = 6
                    ...
                    # Without dead-end
                    self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]), np.array([9.2, 0.2]), np.array([6, 0.5]), np.array([6, 0.5])]
            ...
            for i in range(100000):
                ...
                if self.test_collision(self.goal_pos, 0.6):
                    continue
                else:
                    break
            ...
            for i in range(100000):
                ...
                if np.linalg.norm(self.state[:2] - self.goal_pos) < 2.0 or self.test_collision(self.state[:2], 0.6):
                    continue
                else:
                    break
