...
class CurEnvDivObs(gym.Env):
    ...
    def __init__(self):
        ...
        self.start_pos = np.array([0.0, 0.0, 5.0])
        self.bound_box = np.array([[-15.0, 15.0], [-15.0, 15.0]])

        self.state = np.copy(self.start_pos)
        self.viewer = None

        self.num_samples_laser = 100
    ...
    def _step(self, action):
        ...
        if done:
            ...
            if collision:
                ...
                if np.linalg.norm(self.goal_pos - self.state[0:2]) < self.goal_radius:
                    reward = 1.0
                    done = True
                else:
    ...
    def laser_readings(self):
        ...
        lidar_readings = 1 * np.ones(num_samples)
