...
class PlannerRectangleMap2Observation4(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle','rectangle']
        self.obstacle_pos = np.zeros((self.obstacle_num, 3))
        self.obstacle_dim = []
        self.obstacle_dim.append(self.obstacle_radius)

        # record 4 observation
        self.o_pre1 = np.zeros(70)
        self.o_pre2 = np.zeros(70)
        self.o_pre3 = np.zeros(70)

        # record 4 goal_measurements
        self.goal_pre1 = np.zeros(2)
        self.goal_pre2 = np.zeros(2)
        self.goal_pre3 = np.zeros(3)
    ...
    def _step(self, action):
        ...
        if done:
            ...
            if collision:
                ...
                if np.linalg.norm(self.goal_pos - self.state[0:2]) < 0.5:
                    reward = 1.0
                    done = True
                else:
        ...
        measurement = np.concatenate((observation, self.o_pre1, self.o_pre2, self.o_pre3, goal_measurements, self.goal_pre1, self.goal_pre2, self.goal_pre3)) # After (feed observation first)
        # measurement = np.concatenate((goal_measurements, observation))  # Before (feed goal_measurement first)
        # measurement = goal_measurements


        # record 4 observation
        self.o_pre3 = self.o_pre2
        self.o_pre2 = self.o_pre1
        self.o_pre1 = observation

        # record 4 goal_measurements
        self.goal_pre3 = self.goal_pre2
        self.goal_pre2 = self.goal_pre1
        self.goal_pre1 = goal_measurements
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            ...
            #     if not ((self.goal_pos[1] > 4.5 and self.goal_pos[1] < 5.2) or (self.goal_pos[1] > 1.8 and self.goal_pos[1] < 2.2)):
            #         break
            ...
            # Start pose
            for i in range(50):
                state_x = np.random.uniform(0.5, 8.5, 1)
                state_y = np.random.uniform(0.5, 8.5, 1)

                self.state[0] = math.floor(state_x * 2 + 0.5) / 2
                self.state[1] = math.floor(state_y * 2 + 0.5) / 2

                if np.linalg.norm(self.state[:2] - self.goal_pos) < 2.0:
                    continue
                else:
                    break
            ...
            for it_obstacle in range(self.obstacle_num):
                if self.obstacle_type[it_obstacle] == 'cylinder':
                    ...
                    #
                    self.obstacle_pos[9, 0:2] = [2.0, 7.0]
                    self.obstacle_pos[10, 0:2] = [4.8, 3.5]
                    self.obstacle_pos[11, 0:2] = [7.0, 4.5]
        ...
        # record 4 observations
        self.o_pre1 = observation
        self.o_pre2 = observation
        self.o_pre3 = observation

        # record 4 goal_measurements
        self.goal_pre1 = goal_measurements
        self.goal_pre2 = goal_measurements
        self.goal_pre3 = goal_measurements




        # return np.concatenate((goal_measurements, observation)) # Before
        return np.concatenate((observation, self.o_pre1, self.o_pre2, self.o_pre3, goal_measurements, self.goal_pre1, self.goal_pre2, self.goal_pre3)) # After
