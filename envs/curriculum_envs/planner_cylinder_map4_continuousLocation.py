...
class PlannerCylinderMap4_continuousLocation(gym.Env):
    metadata = {
        ...
        if not self.manual_pose:
            ...
            for it_obstacle in range(self.obstacle_num):
                if self.obstacle_type[it_obstacle] == 'cylinder':
                    ...
                    self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]), np.array([9.2, 0.2]), 0.4, 0.6, 0.2 ]
            ...
            for i in range(100000):
                ...
                # No Rand
                self.state[0] = state_x
                self.state[1] = state_y
