...
class PlannerCylinder4Rectangle1Wall1_continuous(gym.Env):
    metadata = {
    ...
    def _step(self, action):
        ...
        if done:
            ...
            if collision:
                ...
                if np.linalg.norm(self.goal_pos - self.state[0:2]) < 0.3:
                    reward = 1.0
                    done = True
                else:
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            ...
            if self.map_1 == False and self.map_2 == False:
                self.obstacle_num = 7
                self.obstacle_pos = np.zeros((self.obstacle_num, 3))
                self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'cylinder', 'cylinder',
                                      'cylinder']

                self.obstacle_pos[0] = [0, 4.5, 0]
                self.obstacle_pos[1] = [9, 4.5, 0]
                self.obstacle_pos[2] = [4.5, 9, 0]
                self.obstacle_pos[3] = [4.5, 0, 0]
                self.obstacle_pos[4] = [5.0, 3.0, 0]
                self.obstacle_pos[5] = [3.0, 5.0, 0]
                self.obstacle_pos[6] = [7.2, 6.2, 0]

                self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]),
                                     np.array([9.2, 0.2]), 0.4, 0.6, 0.2]

                self.map_1 = True

            elif self.map_1 == True and self.map_2 == False:
                self.obstacle_num = 7
                self.obstacle_pos = np.zeros((self.obstacle_num, 3))
                self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle',
                                      'rectangle']

                self.obstacle_pos[0] = [0.0, 4.5, 0.0]
                self.obstacle_pos[1] = [9.0, 4.5, 0.0]
                self.obstacle_pos[2] = [4.5, 9.0, 0.0]
                self.obstacle_pos[3] = [4.5, 0.0, 0.0]
                self.obstacle_pos[4] = [2.0, 5.0, 0.1]
                self.obstacle_pos[5] = [4.0, 2.5, 0.4]
                self.obstacle_pos[6] = [5.2, 6.3, 2.7]

                self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]),
                                     np.array([9.2, 0.2]), np.array([0.4, 0.4]), np.array([3, 2]), np.array([5, 1])]

                self.map_2 = True

            elif self. map_1 == True and self.map_2 == True:
                self.obstacle_num = 6
                self.obstacle_pos = np.zeros((self.obstacle_num, 3))
                self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle']
                ...
                self.obstacle_pos[2] = [4.5, 9.0, 0.0]
                self.obstacle_pos[3] = [4.5, 0.0, 0.0]
                self.obstacle_pos[4] = [3.0, 3.0, 0.0]
                self.obstacle_pos[5] = [6.0, 6.0, 0.0]

                self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]),
                                     np.array([9.2, 0.2]), np.array([6, 0.5]), np.array([6, 0.5])]
            ...
            for i in range(100000):
                ...
                if np.linalg.norm(self.state[:2] - self.goal_pos) < 2.0 or self.test_collision(self.state[:2],
                                                                                               0.3):
                    continue
                else:
                    break
