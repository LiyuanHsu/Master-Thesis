...
class CylinderDiffSizesEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.start_pos = np.array([-2.0, 0.0])
        self.bound_box = np.array([[-5.0, 5.0], [-5.0, 5.0]])
        ...
        self.objects_pos = np.array([-5.5, 0.0])
    ...
    def _step(self, action):
        ...
        if done:
            ...
            for it_obstacles in range(self.obstacle_num):
                if np.linalg.norm(self.state - self.objects_pos) \
                        < (self.obstacle_radius[it_obstacles] + 0.2):
                    reward = -1.0
                    done = True
                    break
        ...
        # Draw matlab
        Draw_matlab.Draw_matlab(action, observation)
    ...
    def _reset(self):
        ...
        self.objects_pos = np.array([-0.0, 0.0])
        # self.objects_pos = np.array([[-5.5, 0.5], [-7.0, 1.0], [-3.0, 1.5], [-4.5, -1.3]])
