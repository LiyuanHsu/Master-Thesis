...
class PlannerComplexMap5_1(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle']
    ...
    def _step(self, action):
        ...
        # observation = [x - 0.5 for x in observation]
        # observation = [0 if i < 0 else i for i in observation]
        ...
        if not self.manual_pose:
            ...
            for it_obstacle in range(self.obstacle_num):
                if self.obstacle_type[it_obstacle] == 'cylinder':
                    ...
                    # Without dead-end
                    self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]), np.array([9.2, 0.2]), np.array([2, 2]), np.array([2, 2]), np.array([2, 2]), np.array([2, 2])]
