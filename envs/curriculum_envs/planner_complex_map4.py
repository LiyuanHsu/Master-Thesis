...
class PlannerComplexMap4(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.obstacle_num = 13
        self.obstacle_radius = 1
        self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle','rectangle', 'rectangle', 'cylinder', 'cylinder', 'cylinder']
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            ...
            for it_obstacle in range(self.obstacle_num):
                if self.obstacle_type[it_obstacle] == 'cylinder':
                    ...
                    self.obstacle_pos[11, 0:2] = [7.5, 5]
                    self.obstacle_pos[12, 0:2] = [2.6, 3]
                    ...
                    self.obstacle_pos[5, 0] = 6.5
                    self.obstacle_pos[5, 1] = 6.5

                    self.obstacle_pos[6, 0] = 0.5
                    self.obstacle_pos[6, 1] = 4

                    self.obstacle_pos[7, 0] = 4.5
                    self.obstacle_pos[7, 1] = 0.5

                    self.obstacle_pos[8, 0] = 4
                    self.obstacle_pos[8, 1] = 8.5

                    self.obstacle_pos[9, 0] = 8
                    self.obstacle_pos[9, 1] = 3
                    ...
                    # Without dead-end
                    self.obstacle_dim = [np.array([0.2, 9.2]), np.array([0.2, 9.2]), np.array([9.2, 0.2]), np.array([9.2, 0.2]), np.array([3, 5]), np.array([1, 0.5]), np.array([1, 0.5]), np.array([3, 1]), np.array([0.5, 1]), np.array([2, 0.4])]
        ...
        # TODO: can check collision at start of goal and obstacle
            ...
            for i in range(100000):
                ...
                # if not ((np.linalg.norm(self.goal_pos - self.obstacle_pos[4, :2]) < 2.1) or \
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[5, :2]) < 0.6) or\
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[6, :2]) < 0.6) or\
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[7, :2]) < 0.8) or\
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[8, :2]) < 0.6) or\
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[9, :2]) < 1.1) or\
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[10, :2]) < 0.6) or\
                #         (np.linalg.norm(self.goal_pos - self.obstacle_pos[11, :2]) < 0.6)):
