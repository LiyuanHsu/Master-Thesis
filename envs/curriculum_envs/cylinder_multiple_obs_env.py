...
class CylinderMultipleObstEnv(gym.Env):
    metadata = {
    ...
    def _step(self, action):
        ...
        if done:
            ...
            for it_obstacles in range(self.obstacle_num):
                if np.linalg.norm(self.state - self.objects_pos[it_obstacles, :]) < (self.obstacle_radius + 0.4):
                    reward = -1.0
                    done = True
                    break
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            obstacle_x = np.random.uniform(-8.0, -3.0, self.obstacle_num)
            obstacle_y = np.random.uniform(-2.0, 2.0, self.obstacle_num)

            self.objects_pos[:, 0] = obstacle_x
            self.objects_pos[:, 1] = obstacle_y
