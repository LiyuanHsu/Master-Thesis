...
class PlannerComplexMap1(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.obstacle_type = ['rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle', 'rectangle','rectangle', 'cylinder', 'cylinder', 'cylinder']
    ...
    def _reset(self):
        ...
        if not self.manual_pose:
            ...
            for i in range(50):
                ...
                if np.linalg.norm(self.obstacle_pos[9, 0:2] - self.goal_pos) < 0.6 or np.linalg.norm(self.obstacle_pos[10, 0:2] - self.goal_pos) < 0.6 or np.linalg.norm(self.obstacle_pos[11, 0:2] - self.goal_pos) < 0.6:
                    continue
                else:
                    break
            ...
            for i in range(50):
                ...
                if np.linalg.norm(self.state[:2] - self.goal_pos) < 2.0 or np.linalg.norm(self.obstacle_pos[9, 0:2] - self.state[:2]) < 0.6 or np.linalg.norm(self.obstacle_pos[10, 0:2] - self.state[:2]) < 0.6 or np.linalg.norm(self.obstacle_pos[11, 0:2] - self.state[:2]) < 0.6:
                    continue
                else:
                    break
