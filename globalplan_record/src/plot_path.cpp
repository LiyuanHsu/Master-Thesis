...
if(valid_path == true)
{
	...
	if((path_size - path_index) > 40 && get_path == true)
	{
		//file.open ("laser_data.csv");
		advance(it_list_x, 40);
		advance(it_list_y, 40);
...
if(get_path == false)
{
	...
	{				
		...
		std::cout << "Point" << i << ": " << "x:" << it->pose.position.x << ' ' << "y:" << it->pose.position.y << '\n';
 		file << '\n';
		file << i << ',' << it->pose.position.x << ',' << it->pose.position.y; 
		i++;
	}
	if(valid == true)
	{ 	
	...
	//std::cout << "List size: " << path_list_x.size() << "\n";
...
int main(int argc, char **argv)
{
	...
	// csv data
	file.open("plot_path.csv", std::ios_base::app);
	if(!file)
	{
		file.open("plot_path.csv");
		file.close();
		file.open("plot_path.csv", std::ios_base::app);
	}
