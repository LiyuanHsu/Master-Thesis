...
void request_start()
{
	...
	//} while ((start_x == 3 && start_y == 5) || (start_x == 5 && start_y == 3) || (start_x == 7 && start_y == 6));
...
if(get_path == false)
{
	...
	{				
		...
		if (it->pose.position.x != *it_list_x_int || it->pose.position.y != *it_list_y_int)
		{
			path_list_x_int.push_back(it->pose.position.x);
			path_list_y_int.push_back(it->pose.position.y);			
		}
	...
	{ 	
		...
		path_size = data.size();
		if (path_size > 120 && reach_goal == true && trajectory_contain_origin == false)
		{
			valid_path = true;
			set_goal = true;
			publish_count = 0;
		...
		else
		{
			it_path += 1;
			short_path_count += 1;
			if (path_size <= 120) std::cout << "+++ Short Path +++" << '\n';
